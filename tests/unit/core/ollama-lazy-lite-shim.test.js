...
//
// core/ollama.js is never shipped in the Lite tarball, so the real
// ollama-lazy.js's `await import('./ollama.js')` is a literal dynamic-
// import target that would throw ERR_MODULE_NOT_FOUND in an installed Lite
// package if it were ever reached. build.mjs (the Lite package build)
// substitutes THIS file under the exact same path (core/ollama-lazy.js)
// when staging, so every caller's import specifier is unchanged and no
// caller needs to know which variant it's running against.
//
...
describe('core/ollama-lazy.lite.js — Lite staging shim', () => {
  ...
  it('the shim module source contains no ACTUAL (non-comment) dynamic or static import of core/ollama.js', async () => {
    ...
    // real loader's `await import('./ollama.js')` in prose, to explain why
    // the shim exists. Checking the raw source (including comments) would
    // false-positive on that documentation; the real invariant is about
    // executable code only.
    const codeOnly = src.split('\n').filter((line) => !line.trim().startsWith('//')).join('\n');
    assert.ok(!/import\(['"]\.\/ollama\.js['"]\)/.test(codeOnly), 'must not dynamically import ./ollama.js in executable code');
    assert.ok(!/from ['"]\.\/ollama\.js['"]/.test(codeOnly), 'must not statically import ./ollama.js in executable code');
  });
});
