...
// it alive for the duration of the indexer process.
//
...
//   shutdownOnnxTagWorker()  — terminates the worker; call on process exit
...
// (mirrors core/ce-rerank.js's own DI seam). Must return an object shaped
// like a node:child_process ChildProcess: .send(msg), .on(event, fn),
// .off(event, fn), .once(event, fn), .kill(). Config travels via
// environment variables (never IPC, never CLI args) since modelId/cacheDir/
// numThreads/allowDownload never change for this child process's lifetime.
...
async function ensureWorker() {
  ...
  _initPromise = (async () => {
    ...
    // first (the same crash core/ce-rerank.js's own init handler had:
    // "Cannot read properties of null (reading 'off')" when a worker exits
    // during its own init). Using the local `worker` reference throughout
    // this block makes cleanup independent of that ordering.
 ...
 * own module-level comment) — this function's returned promise resolves
...
/**
 * Terminates the tag-onnx worker child process (if running). Call on
 * process shutdown to avoid an open child process keeping the parent alive.
 *
 ...
 * leave that caller's promise pending forever.
 */
...
export const __test = {
  reset() {
    _worker = null; _workerReady = false; _initPromise = null; _nextId = 1;
    _tagModelFailed = false; _tagFailureLogged = false;
    _pending.clear();
    _dispatchTail = Promise.resolve();
