...
**Хмарний пошук документів, відповіді з джерелами та агенти під контролем застосунку.**

[Вибір API](#вибір-api) · [Агентний режим](#агентний-режим-018-експериментальний) ·
[Контракт Ask v3](https://github.com/CodeNoob53/semidex/blob/main/docs/en/agent-api-v3.md)
...
- експериментальний Ask v3 із власними інструкціями та нативними викликами інструментів;
- `semidex-lite/client` для Search та Ask v1/v2/v3 у JavaScript/TypeScript,
  з опційними повторами та зручним помічником `askText()`;
...
Grounded Ask v1/v2 зберігають внутрішні системні інструкції. Для власних
інструкцій та інструментів використовуйте окремий агентний Ask v3.

## Вибір API

| Потреба застосунку | Endpoint | Метод клієнта |
| --- | --- | --- |
| Релевантні фрагменти без генерації | `POST /api/v1/search` | `search()` |
| Одноходова відповідь із посиланнями на джерела | `POST /api/v1/ask` | `askV1()` / `askText()` |
| Відповіді з джерелами та обмеженим контекстом розмови | `POST /api/v2/ask` | `askV2()` |
| Власні інструкції та виконання інструментів застосунком | `POST /api/v3/ask` | `askAgent()` / `agentStep()` |

**Межа інтеграції:** встановіть пакет у проєкт, запустіть
`npx semidex-lite serve` окремим процесом і звертайтеся до нього зі свого
backend. Експортований клієнт працює через HTTP, це не вбудований рушій
індексації чи retrieval. Автентифікація користувачів і дозволи на інструменти
залишаються у вашому backend. Не передавайте integration key у браузер.

### Агентний режим (0.1.8, експериментальний)

Ask v3 використовує нативний function calling Gemini. Один запит виконує один
крок моделі; циклом керує ваш застосунок:

```text
ваш backend -> input + systemInstructions + схеми інструментів
Semidex     -> requires_action + toolCalls + continuationId
ваш backend -> перевірка, дозвіл, виконання; повернення toolResults
Semidex     -> наступний запит інструментів або completed із відповіддю
```

Semidex не виконує інструменти та не підключається до ваших MCP-серверів.
Агентний режим не виконує прихований retrieval і не гарантує citations.
Якщо агенту потрібна база знань, надайте пошук як інструмент застосунку.

Після налаштування Gemini та запуску сервера створіть окремий ключ:

```bash
npx semidex-lite key add --name my-agent --collection "*" --operation agent
```

У середовищі backend задайте `SEMIDEX_URL` з адресою сервера (за замовчуванням
`http://127.0.0.1:8642`) та `SEMIDEX_KEY` з отриманим токеном. Запустіть:

```bash
node node_modules/semidex-lite/examples/agent-tool-loop.mjs
```

[Приклад](./examples/agent-tool-loop.mjs) містить обмежений цикл і синтетичний
read-only пошук, а не живу інтеграцію магазину. Наявні Ask-ключі не отримують
scope `agent` автоматично. Continuation зберігається в пам'яті процесу,
має строк дії та втрачається після перезапуску. Ваш застосунок відповідає за
побічні ефекти й не повинен сліпо повторювати дію після неоднозначного збою.

Схеми, ліміти, streaming-події та правила відновлення описані в
[контракті Ask v3](https://github.com/CodeNoob53/semidex/blob/main/docs/en/agent-api-v3.md).
...
### TypeScript: звуження відомих подій

`askV1()`/`askV2()` повертають об'єднання відомих форм подій плюс
прямосумісний `AskUnknownEvent` для будь-якої майбутньої назви SSE-події.
Спочатку звужуйте guard'ом відповідної версії, потім — `switch` на
`event.type`; невідома подія просто пропускається, а не відхиляється:
...
for await (const event of semidex.askV1({ collection: 'my-docs', question })) {
  if (!isKnownAskV1Event(event)) continue; // прямосумісність: майбутня подія, ігнорується

  switch (event.type) {
    case 'sources': console.log(event.sources); break;
    case 'answer_delta': process.stdout.write(event.text); break;
    case 'done': console.log(event.answer, event.citations); break; // у v1 немає `.conversation`
  }
}
```

Для `askV2()` так само використовуйте `isKnownAskV2Event` — його case
`'done'` додатково несе опційний `event.conversation`. Проста перевірка
`event.type === 'done'` *без* guard'а не звужує тип соundно (`type` в
`AskUnknownEvent` — це `string`, не літерал; див. власний doc-коментар у
`index.d.ts`) — завжди спочатку guard.

### Стрімінг чи зручність: який Ask API обрати?

Обидва працюють з тими самими ендпоінтами й дотримуються тих самих
контрактів; відрізняється лише те, що ви отримуєте.

| | `askV1()` / `askV2()` | `askText()` |
| --- | --- | --- |
| Повертає | асинхронний генератор подій | один `Promise` із готовим результатом |
| Коли обирати | потрібен вивід токен за токеном — чат-UI, SSE-ендпоінт, який ви транслюєте в браузер | потрібна лише готова відповідь — виклик інструмента, пакетне завдання, нестрімінговий HTTP-обробник |
| Ви пишете | цикл `for await`, що сам накопичує `answer_delta` | нічого; накопичення виконується за вас |
| Помилки | `SemidexApiError`, кинутий із циклу | той самий `SemidexApiError`, через reject |

`askText()` — чистий споживач стрімінгових методів: він не додає власної
поведінки запиту, а стрімінгові методи лишаються без змін.

```js
// Ask v1 — одна готова відповідь.
const { answer, sources, citations, done } = await semidex.askText({
  collection: 'my-docs',
  question: 'Який термін повернення?',
});
console.log(answer);              // повний текст відповіді
console.log(citations);           // напр. [1, 3] — індекси (від 1) у `sources`
console.log(done.usage);          // повна термінальна подія `done` теж доступна

// Ask v2 — те саме, плюс підтвердження розмови з події `done`.
const result = await semidex.askText({
  version: 'v2',
  collection: 'my-docs',
  question: 'А які є винятки?',
  conversation: { conversationId, summary, recentMessages },
});
...
`askText()` повертає рівно таку структуру:

```ts
{
  answer: string,        // `answer` з події `done`, або склеєні дельти, якщо сервер його не надіслав
  sources: AskSource[],  // [] якщо потік не містив події `sources`
  citations: number[],
  done: AskDoneEventV1 | AskDoneEventV2,  // уся термінальна подія — usage/timing/provider/refused
  conversation: AskConversationDoneBlock | null,  // ЗАВЖДИ null для Ask v1
}
```

У TypeScript це два окремі, перевантажені типи результату —
`AskTextResultV1` (`done: AskDoneEventV1`, `conversation` статично `null`) і
`AskTextResultV2` (`done: AskDoneEventV2`, `conversation:
AskConversationDoneBlock | null`) — обираються за `version`, тож v1-виклик
неможливо статично сплутати з v2-результатом, а `conversation.updatedSummary`
(опційне поле — відсутнє, а не `null`, коли сервер не перераховував summary)
неможливо прочитати без перевірки наявності.

Невдала генерація ніколи не повертає часткову відповідь: термінальна
SSE-подія `error`, помилка до початку потоку, тайм-аут, скасування чи
пошкоджений кадр — усе це **reject** з `SemidexApiError`, навіть якщо частина
тексту `answer_delta` вже надійшла. Потік, що завершився без термінальної
події `done`, теж є помилкою (`client_incomplete_stream`), а не тихим
частковим результатом.

### Повторні спроби (retries)

Клієнт уміє повторювати невдалий запит з обмеженою експоненційною затримкою.
**Повтори вимкнено за замовчуванням** — типове значення `attempts: 1`, тобто
жодного повтору, тож оновлення пакета ніколи не перетворить тихцем один
запит на кілька і не помножить ваші витрати проти сервера з обмеженням
частоти.
...
const semidex = createSemidexClient({
  ...
  timeoutMs: 45_000,          // ЗАГАЛЬНИЙ бюджет виклику, разом із повторами й затримками
  retry: {
    attempts: 3,              // загальна кількість спроб, не додаткових. 1 = без повторів (типово)
    initialDelayMs: 250,      // база експоненційного розкладу
    maxDelayMs: 8000,         // жорстка стеля БУДЬ-ЯКОГО очікування, включно з Retry-After
    backoffFactor: 2,
    jitter: true,             // рандомізація в межах [половина, повна] обчисленої затримки
    onRetry: (info) => console.warn(`повтор ${info.nextAttempt} через ${info.delayMs}мс (статус ${info.status})`),
  },
});

// Для окремого виклику — накладається на політику клієнта (незадані ключі успадковуються):
await semidex.search({ collection: 'my-docs', query: 'x', retry: { attempts: 5 } });
```

**`search()` і Ask повторюються за РІЗНИМИ правилами, бо Ask не є
ідемпотентним.** Друга спроба Ask заново виконує retrieval *і* заново
генерує токени, витрачаючи бюджет ще раз — тож те саме за формою відмова
навмисно трактується по-різному:

| | `search()` | `askV1()` / `askV2()` / `askText()` |
| --- | --- | --- |
| HTTP `429`, `502`, `503`, `504` | Повторюється | Ніколи не повторюється, ОКРІМ випадку, коли тіло відповіді — розпізнаний JSON-конверт Semidex з `retryable: true` |
| Мережева помилка до будь-якої відповіді | Повторюється | **Ніколи не повторюється** |
| HTTP `400`, `401`, `403`, `404` | Ніколи не повторюється | Ніколи не повторюється |
| HTTP `500` | Ніколи не повторюється | Ніколи не повторюється |
| Будь-що взагалі, щойно почався SSE-потік Ask | н/д | **Ніколи не повторюється** |

`search()` — операція лише читання, тож мережева помилка до будь-якої
відповіді безумовно безпечна для повтору — на сервері нічого ще не
витрачено. Ask — інша справа: обрив з'єднання до відповіді (чи навіть до її
заголовків) справді неоднозначний — сервер міг ніколи не отримати запит, а
міг уже прийняти його й почати генерацію до того, як з'єднання обірвалося
— і з боку клієнта відрізнити одне від іншого неможливо. Повтор ризикував
би саме тим дублюванням генерації, заради запобігання якому й існує вся ця
політика, тож Ask вирішує цю неоднозначність, ніколи не повторюючи голу
мережеву помилку — за будь-якого статусу, без винятків. **Отримання
заголовків відповіді — це не те саме, що «сервер ще не почав генерувати»**
— це лише позначає момент, коли САМЕ ЦЕЙ клієнт перестає бачити «байтів
немає» і починає бачити байти; про стан сервера ДО цього моменту це нічого
не каже.

Єдине, що Ask МОЖЕ повторити, — вужче й іншого роду: справді ОТРИМАНЕ
pre-stream JSON-тіло помилки, чий власний payload прямо каже
`retryable: true` — тобто сам Semidex, уже підтвердивши, що відповів, так і
не відкривши потік, ручається, що повтор безпечний. Звичайний `502`/`504`
від реверс-проксі без JSON-тіла, або з JSON-тілом без поля `retryable`, під
це не підпадає. Це свідоме дзеркальне перевертання правила «не довіряти
серверному `retryable: true`», яке керує рішенням `search()` за кодом
статусу вище (майбутній сервер, що почне позначати якийсь `400` як
retryable, і далі не зможе перетворити помилку валідації Search на шторм
повторів) — для Ask саме явний, ОТРИМАНИЙ `retryable: true` є єдиним
сигналом, достатньо сильним, щоб переважити ризик неідемпотентності, бо він
доводить, що відповідь справді прийшла до того, як почався якийсь потік.

Останній рядок таблиці — найважливіший незалежно від ендпоінта. Щойно
сервер почав транслювати генерацію, жодна помилка не повертається до циклу
повторів — ні термінальна подія `error`, ні пошкоджений кадр, ні обрив
сокета посеред відповіді. Запит, який уже видав байти, ніколи не
надсилається повторно, тож вам ніколи не виставлять рахунок двічі за одне
питання і ніколи не покажуть дубльовану часткову відповідь.

**`Retry-After` враховується, але обмежується стелею.** Коли сервер надсилає
коректний `Retry-After`, він використовується як *нижня межа* наступного
очікування — сервер краще знає власне вікно пропускної здатності, ніж
формула на боці клієнта. Але його ніколи не дотримуються понад `maxDelayMs`:
`Retry-After: 3600` не підвісить ваш запит на годину. Натомість клієнт
припиняє повтори й повертає помилку зі збереженим `err.retryAfterSeconds`,
щоб *ви* вирішили, чи планувати повтор через годину — це рішення про
планування, а не про HTTP.

**Тайм-аути й скасування переривають затримку.** `timeoutMs` — це загальний
бюджет реального часу на весь виклик, що покриває кожну спробу *й* кожен сон
між ними; увімкнення повторів ніколи не дасть виклику пережити цей бюджет.
Для `askV1()`/`askV2()` цей бюджет покриває й **повне читання SSE-потоку до
кінця** — не лише з'єднання й перший байт. Повільна або незвично довга
генерація обмежена тим самим `timeoutMs`, тож для комбінації модель/промпт,
яка очікувано буде довгою, підвищуйте його (клієнтський чи per-call), а не
покладайтеся на дефолтні 60с. `AbortSignal` або тайм-аут, що спрацював, поки
клієнт чекає в затримці, негайно завершує виклик звичайною помилкою
`client_timeout_or_abort`, замість того щоб досипати решту затримки.

**Спостережуваність.** Активність повторів видно без колбека і без розкриття
чогось чутливого: успішний результат несе `result.retries`, а кинута помилка
— `err.retries` (обидва неперелічувані, тож форми відповіді й помилки
незмінні). Необов'язковий колбек `onRetry` отримує `{ attempt, nextAttempt,
delayMs, delaySource, status, code, retryAfterSeconds, reason }` — лише коди
статусів і час, ніколи не заголовок, не тіло запиту й не bearer-токен.
...
### Інтеграція з браузерного застосунку: обов'язкова архітектура

```text
браузер (без секретів)  -->  ВАШ backend (тримає SEMIDEX_TOKEN, володіє зіставленням колекцій)  -->  Semidex Lite
```

> **Ніколи не розміщуйте ключ Semidex у браузерному коді.** Ні в теґу
> `<script>`, ні у змінній середовища, яку розкриває збирач (`VITE_*`,
> `NEXT_PUBLIC_*`, `REACT_APP_*`), ні у бінарнику мобільного застосунку, ні
> у «тимчасовій» налагоджувальній збірці. Усе, що потрапляє на клієнтський
> пристрій, є публічним: будь-хто, хто відкриє devtools, отримує весь ваш
> бюджет на читання й генерацію для цієї колекції, і єдиним засобом
> залишиться ротація ключа. Integration API — це інтерфейс
> **backend-to-backend**. Ваш браузерний код звертається до *вашого*
> сервера; лише ваш сервер тримає ключ і викликає Semidex.

З цього випливають дві речі, і обидві продемонстровані у прикладах, що
постачаються з пакетом:

**1. Зіставляйте прикладний `assistantId` з колекцією на backend.** Браузер
не повинен ані надсилати назву колекції, ані взагалі її знати. Він надсилає
ідентифікатор, зрозумілий *вашому* застосунку; ваш backend володіє явним,
закритим зіставленням на реальну колекцію:

```js
const ASSISTANTS = Object.freeze({
  support:  { collection: process.env.SEMIDEX_SUPPORT_COLLECTION,  label: 'Підтримка клієнтів' },
  handbook: { collection: process.env.SEMIDEX_HANDBOOK_COLLECTION, label: 'Довідник співробітника' },
});

function resolveAssistant(assistantId) {
  // hasOwnProperty, а не звичайний пошук: "__proto__"/"constructor" інакше
  // повернули б істинне значення, яке не є колекцією.
  if (typeof assistantId !== 'string' || !Object.prototype.hasOwnProperty.call(ASSISTANTS, assistantId)) {
    throw Object.assign(new Error('Невідомий assistantId'), { status: 400 });
  }
  return ASSISTANTS[assistantId];
}
```

Приймання «сирої» назви `collection` від браузера дозволило б будь-кому
читати кожну колекцію, на яку виданий ключ, — це вада розкриття даних навіть
за ідеально секретного ключа. Продакшн-застосунок визначає це з дозволів
**автентифікованого** користувача, а не з жорстко заданої мапи; суть у тому,
що зіставлення живе на сервері, є явним і закритим.

**2. Зберігайте стан розмови самостійно.** Semidex Lite не зберігає нічого
між запитами. `summary`/`recentMessages` для Ask v2 надходять від вас на
кожному ході, а подія `done` повертає те, що треба зберегти для наступного —
повний контракт володіння див. у розділі Ask v2 нижче. У прикладах
використано `Map` у пам'яті процесу з явною позначкою **лише для демо**:
він губиться при перезапуску, не поділяється між репліками і росте
необмежено. У продакшні замініть його на те, що вже зберігає ваші дані
користувачів (PostgreSQL, Redis, SQLite, ...), з ключем за вашими власними
ідентифікаторами користувача/сесії та вашою політикою зберігання й
видалення. Тримайте також власне обмеження на вікно останніх повідомлень:
кожне збережене повідомлення надсилається заново на кожному ході, тож
необмежене вікно тихо збільшує вартість промпту кожного запиту.

**Два повні робочі приклади** постачаються в цьому пакеті; вони однакові в
поводженні з ключем, зіставленні колекцій та володінні розмовою — і
відрізняються лише стрімінгом:

| Приклад | Ask API | Браузер отримує |
| --- | --- | --- |
| [`examples/backend-integration-server.mjs`](examples/backend-integration-server.mjs) | `askV2()` | потік Server-Sent Events, токен за токеном |
| [`examples/backend-chat-server.mjs`](examples/backend-chat-server.mjs) | `askText()` | одну JSON-відповідь на хід |

Запустіть будь-який із них проти власного `semidex-lite serve`:

```bash
npx semidex-lite key add --name demo-backend --collection my-docs --operation search --operation generate

# Стрімінговий (SSE) варіант — POST /api/chat, POST /api/search
...
# Нестрімінговий варіант на askText() — POST /chat, GET /assistants
...
Змінні середовища, які читають обидва приклади: `SEMIDEX_TOKEN`
(обов'язкова), `SEMIDEX_BASE_URL` (типово `http://127.0.0.1:8642`), `PORT`
та по одній змінній `SEMIDEX_*_COLLECTION` на кожного налаштованого
асистента.

Оголошення типів (`.d.ts`) постачаються разом із клієнтом для редакторів/
`tsc` — без кроку збірки, без компіляції TypeScript у цьому репозиторії. Тест-
сюїт репозиторію стежить за дрейфом цих оголошень: строга фікстура-споживач
`tsc --noEmit` (`tests/types/`) перевіряє `askV1()`, `askV2()`, звуження
подій і опційність v2-конверсації, і валить збірку на будь-якій регресії —
див. `npm run typecheck:lite-client`.
...
Для grounded Ask v1/v2 системний prompt є внутрішнім: його не можна змінити
через адмін-панель, `.env` або запит. Ask v3 приймає власні
`systemInstructions`, але не успадковує описаний вище контракт відповідей
на основі джерел.
