...
- **Coherent section/file context around a hit, bounded:**
  ...
  cannot use `qdrant_get_content` — reindex to get skeleton node identity.
...
- Markdown files always index through the skeleton chunker with navigation nodes generated automatically — this is architecture, not an opt-in flag, and enables the skeleton navigation tools (`qdrant_get_skeleton` etc.) with no extra configuration. Non-Markdown formats (PDF, Pandoc-converted formats, plain text) still use the legacy chunker and have no skeleton node identity.
