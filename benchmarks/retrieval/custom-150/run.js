// Tier B quality benchmark — custom-150.
//
// Uses bench-retrieval-custom-150 collection. Reuses fixture docs from both
// the shared regression corpus and the custom-50 fixtures directory.
// New fixtures go in custom-150/fixtures/docs/ as coverage gaps are found.
//
// Usage:
//   node benchmarks/retrieval/custom-150/run.js
//   npm run bench:custom150
//   BENCH_PROVIDER=onnx npm run bench:custom150
//   BENCH_SKIP_INDEX=1 npm run bench:custom150
//   BENCH_TOP_K=10 npm run bench:custom150
//   RERANK_ENABLED=1 npm run bench:custom150

if (process.argv.includes('--help')) {
  process.stdout.write(`semidex custom-150 Tier B benchmark

Usage:
  node benchmarks/retrieval/custom-150/run.js [options]
  npm run bench:custom150
...
Options (env vars):
  ...
  RERANK_PREFETCH_MULT=<n>     Prefetch multiplier for reranking (default: 4)
...
Fixtures (reused, no duplication):
  benchmarks/retrieval/fixtures/docs/           (shared: providers, qdrant, chunking, sync)
  benchmarks/retrieval/custom-50/fixtures/docs/ (custom-50 own: mcp-workflow, obsidian, etc.)
...
// Shared: 4 regression fixtures. Custom-50: 6 own fixtures.
// Custom-150 own: any .md files in fixtures/docs/ if it exists.
...
function envInt(name, def, min, max) {
  ...
  if (!Number.isFinite(v) || v < min || v > max) {
    if (process.env[name] !== undefined)
      console.warn(`[bench-c150] ${name}="${process.env[name]}" is invalid — using default ${def}`);
    return def;
  }
  return v;
}
...
function printResults(queryResults) {
  const W = { id: 8, query: 44, cr3: 4, cr5: 4, sup: 4, ndcg: 7, mrr: 6, ms: 6 };
  const header = [
    pad('ID',    W.id),
    pad('Query', W.query),
    lpad('cr3',  W.cr3),
...
async function main() {
  ...
  if (typeWarnings.length) {
    for (const w of typeWarnings) process.stderr.write(`[bench-c150] type warning: ${w}\n`);
  }
  ...
  // fresh — never a second, independent env read after this point. This is
  // what makes BENCH_SKIP_INDEX=1 safe: PROFILE always reflects the
  // collection's REAL recorded identity, not whatever the current shell
  // happens to have set.
  const { denseProvider, sparseProvider } = await ensureCollection();

  log(`\n=== semidex custom-150 Tier B benchmark ===`);
  ...
  log(`Fixtures  : ${FIXTURE_FILES.length} docs`);
