...
| 9 | **Rate-limit identity (implemented)** | per IP / per key / per key+route | **Per key** (`principal.keyId`), one bucket shared across every route the key can reach | IP is wrong behind a wrapper backend (every request shares one IP). `costClass` bucketing was the original recommendation, but was not adopted: the Integration audience originally had exactly one route pair (Ask v1/v2), both `costClass: llm`, so a second dimension would have added complexity with no route pair to distinguish. **Revisited (2026-08-24):** `POST /api/v1/search` shipped as a second Integration route with a DIFFERENT `costClass` (`qdrant`, never `llm` — Search never calls a generation provider), exactly the trigger this row named. Decision confirmed unchanged: Search shares ONE bucket with Ask, per key, not a second per-`costClass` bucket — a backend that does both Search and Ask still has one effective rate budget to reason about, and `costClass`-aware rate limiting remains open for a future phase if a real need for differentiated throughput per operation type emerges (spend is already bounded separately and per-`costClass` for Ask by §12m's token-budget ledger, which Search does not need since it has no generation spend to bound). |
