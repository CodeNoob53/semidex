...
> **Переглянуто 2026-06-10 (cost-математика реального корпусу).** Початковий
> план «LLM-контекст на кожну сутність + nav-summary» НЕ дешевший за legacy на
> code-важких корпусах: 65 сутностей + 18 nav-вузлів = 83 виклики проти 49 у
> legacy. Рішення: **детермінований контекст за замовчуванням, 0 LLM-викликів**:
>
> - paragraph: `context = heading_path` («Тема › Розділ») — локаційний сигнал;
> - сутність: `context = heading_path — node_type — останнє речення сусідньої
>   прози` (тієї самої, що несе плейсхолдер сутності — звʼязок уже відомий
>   чанкеру);
> - nav-вузли: інвентарні summary (вже реалізовано в buildFileSkeleton).
>
> `SKELETON_CONTEXT=llm` повертає legacy-шлях per-chunk LLM-контексту для
> Stage-3 A/B порівняння. Якщо бенч покаже суттєву втрату якості від
> детермінованого контексту — рішення переглядається на рівні сутностей
> (LLM лише для сутностей > N токенів), але не раніше вимірювання.
> Операційний бонус: skeleton-індексація без TAG_GEN (або з TAG_PROVIDER=onnx)
> взагалі не потребує запущеної Ollama.

### 12.1 Summary секції / файлу / колекції — закешований контекст (виправлено 2026-06-10)

**Концепт (зафіксовано):** summary — це НЕ етикетка карти, а **заздалегідь
оплачена відповідь на питання «про що це?»**. Агент, що заходить у колекцію,
читає готове summary колекції → спускається → читає summary файлу → знає,
про що файл, НЕ відкривши файл. Альтернатива — «побігай по дереву і склади
summary сам» — спалює токени, час і вікно контексту кожного агента при
кожному вході, і для сторонніх моделей це прямі гроші. Тому семантичні
summary — **ядро навігаційного шару, а не опція**.

Економіка, чому це сумісно з відмовою від per-chunk LLM-контексту:

| Що | Викликів (реальний корпус) | Коли платиться |
|----|---------------------------|----------------|
| per-chunk контекст (скасовано §12) | 139–1095 на корпус | при КОЖНОМУ реіндексі чанка |
| nav-summary (цей розділ) | файли+секції: 18 / ~98 | ОДИН раз; кеш по file_hash — файл не змінився → не перегенерується |

Вартість читання агентом: O(1) — один nav-вузол замість обходу піддерева.
Інвестиція окупається з першого ж входу будь-якого агента.

Механіка генерації — адаптивна, правило: **«бери найточніше джерело, яке
влазить у вікно моделі»** (зафіксовано 2026-06-10):

- **секція** ← LLM від: повний текст секції, якщо влазить; інакше lead-чанк
  + інвентар дітей + контексти сутностей;
- **файл** — два шляхи за розміром:
  1. файл влазить у вікно цілком → LLM від ПОВНОГО вмісту файлу
     (найточніше: модель бачить звʼязки між секціями, які roll-up губить);
  2. не влазить → LLM від секційних summary (вони вже готові й короткі);
- **колекція** ← завжди LLM від файлових summary; інкрементально (§9) —
  при зміні одного файлу перегенерується його гілка + колекційний roll-up.

**Крайній випадок «Війна і мир одним файлом»:** навіть секційні summary
можуть не влізти у вікно (300 секцій × 60 ток. = 18k проти бюджету 8k).
Рішення — те саме правило рекурсивно (ієрархічне згортання): список
секційних summary ріжеться на пачки розміром з вікно → проміжне summary
на пачку («про що ця частина») → фінальне summary з проміжних. Кожен
рівень стискає у ~50–100×, тож одного додаткового рівня вистачає на
будь-який документ; вартість — +N_пачок+1 викликів, одноразово.
Бюджет вікна — env `SUMMARY_WINDOW_TOKENS` (консервативний дефолт під
локальну модель, напр. 8000), не хардкод.

Рівні готовності:

1. **MVP (реалізовано):** інвентарні summary (`"Setup — 2 paragraphs,
   1 code_block"`) — тимчасовий каркас, щоб механіка карти і payload-схема
   працювали до підключення LLM-summary. НЕ кінцевий стан.
2. **Stage 2 (ціль):** семантичні LLM-summary за замовчуванням, коли Ollama
   доступна; інвентар — лише fallback без LLM. Lead-речення — компонент
   промпту, не заміна.

Nav-вузли виключені з default-пошуку фільтром `point_kind`; embedding —
локальний ONNX. Якість summary міряється Stage-3 агентським флоу
(«зорієнтуйся в незнайомій колекції»), а не chunk-recall метриками.
