...
| `ONNX_EXECUTION_PROVIDER=cuda` falls back to CPU | CUDA unavailable or not supported on this platform | On Windows, CUDA is not supported via prebuilt npm — use `dml` instead. On Linux x64 + NVIDIA, CUDA is opt-in/experimental: install CUDA 12.x + cuDNN 9, set `LD_LIBRARY_PATH`, then retry. The silent fallback to CPU is a known gap — check stderr for `retrying with cpu` to confirm whether CUDA actually loaded. A planned Stage 1 patch will add a CUDA session probe to `npm run doctor`; a planned Stage 2 patch will add `ONNX_CUDA_STRICT=1` to hard-fail on CUDA load errors. |
