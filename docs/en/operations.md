...
| `ONNX_EXECUTION_PROVIDER=cuda` falls back to CPU | On Windows, the standard npm-installed `onnxruntime-node` package has no CUDA execution provider compiled in (CPU/DirectML only) — see [configuration.md's platform comparison](configuration.md#platform-comparison-cuda-support), since Linux x64 differs. Or CUDA itself is unavailable on the machine. | On Windows, install a **managed** runtime (`.\scripts\install-onnxruntime-cuda-windows.ps1` — see [windows-cuda-onnxruntime-install.md](windows-cuda-onnxruntime-install.md)) or point `ONNXRUNTIME_NODE_PATH` at a compatible **custom** build (see [configuration.md](configuration.md#onnxruntime_node_path--a-custom-runtime-for-cuda-on-windows)); without either, use the verified `dml` path instead. On Linux x64 + NVIDIA, the npm package already ships CUDA support upstream (CUDA 12.x + cuDNN 9), but semidex itself has not run its own end-to-end acceptance pass on that path — treat it as experimental within semidex specifically. Verify the *effective* provider — never assume it from the setting — with the Admin UI's "Test CUDA configuration" button (Embeddings & hardware) or `npm run doctor` with `ONNX_EMBED=1` (both run an isolated CUDA session probe: PASS/WARN, no indexing run, never silently substitutes CPU). A successful probe does not turn Linux into a semidex-supported platform, and a successful Windows probe applies only to the tested environment, not every Windows/NVIDIA combination — end-to-end validation on your own machine is still required. To make CUDA failure a hard error instead of a silent CPU retry during normal indexing/search, set `ONNX_CUDA_STRICT=1`. |
