...
| `ONNX_EXECUTION_PROVIDER=cuda` falls back to CPU | The standard npm-installed `onnxruntime-node` package has no CUDA execution provider compiled in on any platform, or CUDA itself is unavailable | On Windows, CUDA requires a compatible **custom** `onnxruntime-node` build — set `ONNXRUNTIME_NODE_PATH` to it (see [configuration.md](configuration.md#onnxruntime_node_path--a-custom-runtime-for-cuda-on-windows)); without one, use the verified `dml` path instead. On Linux x64 + NVIDIA, CUDA is experimental / unverified: install CUDA 12.x + cuDNN 9, set `LD_LIBRARY_PATH`, then retry. Verify the *effective* provider — never assume it from the setting — with the Admin UI's "Test CUDA configuration" button (Embeddings & hardware) or `npm run doctor` with `ONNX_EMBED=1` (both run an isolated CUDA session probe: PASS/WARN, no indexing run, never silently substitutes CPU). A successful probe does not turn Linux into a supported platform, and a successful Windows probe applies only to the tested environment, not every Windows/NVIDIA combination — end-to-end validation on your own machine is still required. To make CUDA failure a hard error instead of a silent CPU retry during normal indexing/search, set `ONNX_CUDA_STRICT=1`. |
