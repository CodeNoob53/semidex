...
| Variable | Default | Description |
|----------|---------|-------------|
| `SKELETON_CHUNKING` | `0` | Set to `1` to enable skeleton-first chunking for Markdown: tables, code blocks, and checklists become typed structural chunks. Switching this flag on an existing collection triggers a controlled reindex |
| `SKELETON_NAV` | on | Navigation nodes (collection/directory/file/section) are generated by default for skeleton files. Set to `0` as a kill-switch to skip nav points |
| `SKELETON_CONTEXT` | deterministic | Context for skeleton chunks is deterministic (heading path + structural carryover, no LLM calls). Set to `llm` to restore per-chunk LLM context generation |
